...
# from ims.config.settings import settings
...
#             # if dictionary:
#             #     cursor = _DictCursorWrapper(cursor)
#             return cursor
...
class DatabaseConnection:
    ...
    def fetch_all(self, sql, params=None, *, dictionary=False):
        ...
        try:
            ...
            # If dictionary=True but results are tuples, convert them
            if dictionary and results and isinstance(results[0], tuple):
    ...
    def fetch_one(self, sql, params=None, *, dictionary=False):
        ...
        try:
            ...
            print(f"DEBUG FETCH_ONE: dictionary={dictionary}, result type={type(result)}, result={result}")
            # If dictionary=True but result is a tuple, convert it
            if dictionary and result and isinstance(result, tuple):
                columns = [desc[0] for desc in cursor.description]
                converted = dict(zip(columns, result))
                print(f"DEBUG FETCH_ONE: Converted tuple to dict: {converted}")
                return converted
            return result
        finally:
            self.close_cursor(cursor)
    ...
    def close_connection(self):
        try:
            if self.db_connection and self.db_connection.is_connected():
                self.db_connection.close()
                messagebox.showinfo("Database connection closed",
